...
if (!is_null(filter_input(INPUT_GET, "send"))) {
    $chosedFile = filter_input(INPUT_GET, "chosedFile", FILTER_SANITIZE_STRING);

    for ($i=0; $i < count($listFiles); $i++) { 
        if($listFiles[$i] == $chosedFile){
            $errors="Valid file";
...
<body>
    ...
    <form action= "<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" name="form-count" method = "get">
        ...
        <label for="choseFile">Chose a file name:</label>
        <input type="text" name="chosedFile" value ="<?php echo $inputFile ?>">
